...
# (Use the Pegasos best you observed: T=25, L=0.01)
# ------------------------------------------------------------------------------
if __name__ == "__main__":
    BEST_ALGO = "pegasos"
    BEST_T = 25
    ...
    # ------------------------------------------------------------------------------
    # Most explanatory unigrams (positive + negative)
    # ------------------------------------------------------------------------------
    best_theta = theta
    # Wordlist aligned to theta indices
    wordlist = [word for (idx, word) in sorted(zip(dictionary.values(), dictionary.keys()))]

    # Top positive (highest weights) via provided helper
    sorted_word_features = utils.most_explanatory_word(best_theta, wordlist)
    print("Top 10 positive unigrams:")
    for k, w in enumerate(sorted_word_features[:10], 1):
        print(f"  Top {k}: {w}")

    # Top negative (most negative weights)
    pairs = list(zip(best_theta, wordlist))  # (weight, word)
    pairs.sort(key=lambda t: t[0])           # ascending by weight
    top10_negative = [w for _, w in pairs[:10]]
    print("Top 10 negative unigrams:")
    for k, w in enumerate(top10_negative, 1):
        print(f"  Neg {k}: {w}")
